...
network, a routing node needs to have $N_{in} = N_{out} + F$ Bitcoin allocated
...
\begin{itemize}
    ...
    \item $U_{O}$ is the current UTXO of the orchestrator, where they may
...
\texttt{InitChain($1^{\lambda}$)} $\rightarrow$ $(U_{O}, P_{O}, \Delta_F,
...
\texttt{LiftUTXO($T_{expiry}, \{U_{N_0}, \ldots, U_{N_n}\}, P_{O}$)}
...
long-term public key of the orchestrator $P_{O}$, outputting a new
...
$B_{height}$ is the block height at which the user wishes to exit the chain, the
\texttt{ExitChain} method spends an existing lifted UTXO and resolves the user's
funds in a regular unencumbered UTXO. Users will use this
...
algorithm returns $b=1$ if the operator was able to successfully obtain all
necessary witnesses, and broadcasts the Bitcoin transaction that commits the
...
the shadowchain, each user of this new functionality must update their
...
today, without any further modifications or enhancements.
...
method, and the maker uses the canned \texttt{leasePoint} as the input to the
...
Schnorr signature. As we also want to have our order tagging scheme be secure
...
critical that Lightning Pool reduces information asymmetry for the buyer by
...
posted price and available supply, we employ a multi-attribute match making
...
itself. In addition to these inputs which can only be spent once each user has
...
\begin{itemize}
    ...
    \item In order to synchronize the creation of funding shims and the channel
        funding protocol itself, the auctioneer sends the
        \texttt{BatchSignBegin} message. After sending the  \texttt{BatchAccept} 
        message, the \emph{taker} will register a funding shim to ensure it's 
        able to properly handle the incoming funding request in the next phase.
    ...
    \item The final step for the auctioneer is to commit the new batch to disk,
...
Similar to Lightning Loop, the Lightning Pool backend server is closed
