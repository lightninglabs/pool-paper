...
front-running and other undesirable side effects. Rather than participants
...
as non-interactive signature aggregation (cite) and covenants (cite) could serve
to dramatically improve scalability properties of shadowchains.
...
zero knowledge proofs (cite shafi)) that the resulting UTXO state was properly
derived from the known state transition function.  Note that due to this
structure, complex "exit-games", or fraud proofs are not required as a user
simply won't sign off on a fraudulent state, and a user's UTXO is always
...
cut-through (cite), but is performed in a multi-party setting. Leveraging this
technique, the shadowchain orchestrator can optimistically treat the current
latest shadowchain transaction in the mempool as an in-memory data structure to
be updated off-chain (via transaction replacement techniques (cite)), with the
state being "written to disk" once confirmed. As a result, it's possible to
commit several shadowchain states (possibly hundreds) in a single logical
Bitcoin transaction.
...
Fidelity Bond (cite) with an application-specific twist. This process is akin
to creating a new 'account' within a Shadowchain. The time-lock release nature
of the UTXO means that a user can always recover funds if the
